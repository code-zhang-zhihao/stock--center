...
| 每日事实沉淀、逐表字段与关联关系 | [每日事实沉淀数据字典](20-核心流程/每日事实沉淀数据字典.md) |
...
| 因子计算 | `python-back/app/modules/indicator_engine/` | `backfill.py`、`repository.py`、`service.py` | 个股/板块/指数类型化因子和动态技术快照输入计算。 | [历史数据资产初始化](20-核心流程/历史数据资产初始化.md) |
| 市场洞察 | `python-back/app/modules/market_insight/` | `emotion_service.py`、`report_service.py`、`scheduler_handlers.py` | 从正式日频事实生成双分情绪、周期、龙头与涨停关联证据；不调用 Provider 或 LLM。 | [市场洞察模块](30-模块地图/市场洞察模块.md) |
| 策略研究与模拟交易 | `python-back/app/modules/strategy_center/` | `builtins.py`、`service.py`、`realtime_service.py`、`scheduler_handlers.py` | 固定内置策略、不可变版本、盘后候选、日频基线回测、T+1 纸面确认/退出和审计；不连接券商或自动交易。 | [策略中心模块](30-模块地图/策略中心模块.md) |
...
| 数据库演进 | `docs/sql/` | `README.md`、`77-daily-assets-final-prepare.sql` 至 `80-daily-assets-final-cleanup.sql` | Schema、最终日频资产迁移、索引和流水线升级顺序。 | [数据库表索引](30-模块地图/数据库表索引.md) |
