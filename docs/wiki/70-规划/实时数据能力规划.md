...
盘中实时 Quote 与五档不做全市场 PostgreSQL 沉淀；MooTDX 分钟缓存用于实时展示，15:30 `daily_close_minute_ingest` 才将当日分钟线和分钟因子写入 PostgreSQL 并执行 30 个交易日保留。
...
- **界面与目录 API**：数据中心展示额度、四个状态块、分钟目标分布和统一轮次市场总览。`/market-overview` 通过同一缓存和 SSE 展示概念、行业、股票池、宽度时间线和短期事件；`/post-close-market` 读取完成交易日的涨停梯队、炸板和 `/market-insights/emotion-daily` 正式双分情绪。服务端用 `t_provider_ingest_audit` 验证事件完成，22:15 写 `t_market_emotion_daily`；不足时返回 pending。页面不消耗额外 TickFlow 额度或调用 LLM。股票池在新建/编辑弹窗保存实时策略，目录 API 按系统池、策略池、用户池、题材和行业切换。
...
- `limit_up_count/limit_down_count`：优先采用 TickFlow Quote 返回的涨跌停价；当前 Quote 未返回时，由 `realtime_market/a_share_limit.py` 按证券主数据、前收和交易所规则推导。主板为 10%，科创板/创业板为 20%，北交所为 30%；结果按 `0.01` 元最小变动单位四舍五入，低价股至少跨一个价位。新股上市后前五个开市日、缺少上市日或无法识别板块的 Quote 明确不统计，绝不按 `9.9%` 阈值猜测。
...
- `limit_up_count/limit_down_count`：使用同一份 Provider 或 A 股规则推导的涨跌停价；无法确定证券板块、上市首五日状态或前收时池汇总明确不可用。
