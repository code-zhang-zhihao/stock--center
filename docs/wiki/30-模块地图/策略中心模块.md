...
策略、历史回测和情绪趋势扩散统一读取 `v_stock_factor_daily_active_basis`。V1 激活时视图的 basis 价格为空，消费者回退 BFQ `t_daily_bar`，行为与旧版一致；V2 激活时 OHLC、前收和 MA 均使用 QFQ，突破、均线排列、回踩以及前 20 日高低点不得混入 BFQ 价格。个股日 K 页面仍展示 BFQ K 线，并单独计算 BFQ 显示均线，不复用策略 QFQ MA。
...
`run_strategy_backtest` 是日频、单信号独立样本基线：T 收盘筛选，T+1 开盘用当前激活因子集同口径开盘价与滑点模拟入场，随后以同口径每日收盘判定止损/止盈/移动止盈/期限，下一开市日开盘模拟退出。V2 使用 QFQ OHLC，V1 回退 BFQ；两种价格不能在同一笔交易中混用。它不模拟组合资金容量、盘口排队、撮合、停牌可成交性或真实订单，因此只能用于比较策略版本，不能声称为实盘收益预测。
