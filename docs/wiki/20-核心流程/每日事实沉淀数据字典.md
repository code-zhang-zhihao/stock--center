...
每日任务只传 `trade_date` 调用一次全量接口，不按 `ts_code` 分批。Provider 返回后按原始板块代码去重并与 `t_sector_basic` 本地映射；未映射 Provider 板块和未返回目标板块的数量及最多 100 个代码样本进入任务警告/轻量审计，不触发同轮缺失代码重试。目标覆盖率达到 90% 时视为完成；低于阈值时已映射行仍会入库，审计标为 `ths_daily_coverage_below_threshold`，次日修复按同一交易日重新全量拉取。
...
| `volume`、`amount_yuan` | 成交量；Provider 返回 `amount` 时才归一为元。当前实测单日全量响应未提供 `amount`，因此该列允许为空。 |
...
| `t_sector_bar` | **2026-07-29** | 933 | **事实仍待补齐；已定位为旧的缺失代码重试把首轮成功结果整体回滚，代码已改为单日一次全量请求，部署后需重跑增强/修复任务。** |
...
1. `t_sector_bar` 停在 2026-07-29，是当前每日资产最明显的事实缺口。根因已定位并在代码中修复：旧实现三批请求成功后，会对缺失代码追加一次请求；缺失集合返回空时触发 Token 轮换并使整个独立事务回滚。新实现改为按交易日一次全量请求并保留已成功行；生产事实仍需在部署后通过增强或修复任务补齐。
