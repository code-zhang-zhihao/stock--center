...
新库按顺序执行 `55/56/57` 后，再由目标表 owner 执行 `docs/sql/72-stock-daily-asset-convergence-v2.sql`、`75-stock-factor-v2-window-performance.sql` 和 `76-stock-factor-v2-price-basis-consumers.sql`，随后 reload Scheduler。`72/75/76` 只新增或优化影子资产、视图和任务定义，不删除旧事实；完成 2020-01-01 起的个股事实预热、2021-01-01 起的 V2 因子回填以及至少 5 个完整交易日影子观察后，只有门禁通过才执行 `73-activate-stock-daily-factor-v2.sql`。当前生产库已经通过门禁并激活 V2，V1 已归档；需要语义回滚时执行 `74-rollback-stock-daily-factor-v1.sql`。
