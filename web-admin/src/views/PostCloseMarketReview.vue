...
async function loadReview(silent = false) {
  ...
  try {
    const [structureResult, sentimentResult, emotionResult, reviewResult] = await Promise.allSettled([
      realtimeMarketApi.postCloseStructure(),
      marketInsightApi.dailySentiment(),
      marketInsightApi.emotionDaily(),
...
@media (max-width: 1280px) { .review-summary { grid-template-columns: repeat(3, minmax(0, 1fr)); }.emotion-v2-summary{grid-template-columns:repeat(2,minmax(0,1fr))}.sentiment-layout { grid-template-columns: 130px 1fr; }.sentiment-facts { grid-column: 1 / -1; }.component-grid, .theme-grid { grid-template-columns: repeat(3, minmax(0, 1fr)); }.review-grid { grid-template-columns: 1fr; } }
@media (max-width: 760px) { .post-close-review-page { padding: 14px; }.topbar { flex-direction: column; }.header-actions { justify-content: flex-start; }.round-meta { text-align: left; }.review-summary, .emotion-v2-summary, .emotion-scorecard-grid, .emotion-metric-list, .emotion-details-table, .theme-grid, .evidence-list { grid-template-columns: 1fr; }.ladder-row { grid-template-columns: 54px minmax(0, 1fr); }.ladder-stocks > span { min-width: 92px; } }
</style>
