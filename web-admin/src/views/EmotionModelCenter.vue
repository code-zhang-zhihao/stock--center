  ...
  <main class="workspace emotion-model-page">
    ...
    <n-spin :show="loading">
      ...
      <section v-if="selected" class="surface validation-surface">
        <div class="panel-heading"><div><span class="panel-kicker">校准与历史环境验证</span><h2>基线结果</h2></div><span class="muted">验证只读已持久化评分与事实，不会改写模型</span></div>
        <div class="validation-grid"><article><span>处理交易日</span><strong>{{ calibrationValue('baseline_trade_days_processed') }} / {{ selected.baseline_trade_days }}</strong></article><article><span>可评分日</span><strong>{{ validation?.eligible_score_days ?? calibrationValue('ready_or_degraded_days') }}</strong></article><article><span>短线分均值</span><strong>{{ calibrationValue('short_term_score_average') }}</strong></article><article><span>短线 T+1 末日广度</span><strong>{{ outcomeText(validationPanels[0]?.data?.average_market_breadth_pct, '%') }}</strong></article><article><span>短线 T+3 核心指数累计</span><strong>{{ outcomeText(validationPanels[1]?.data?.average_core_index_cumulative_return_pct, '%', 2) }}</strong></article><article><span>风险偏好 T+1 末日广度</span><strong>{{ outcomeText(validationPanels[2]?.data?.average_market_breadth_pct, '%') }}</strong></article><article><span>风险偏好 T+3 核心指数累计</span><strong>{{ outcomeText(validationPanels[3]?.data?.average_core_index_cumulative_return_pct, '%', 2) }}</strong></article><article><span>校准状态</span><strong>{{ calibrationStatus }}</strong></article></div>
        <div v-if="stageDays.length" class="stage-days"><span v-for="item in stageDays" :key="item[0]">{{ stageLabel(item[0]) }} {{ item[1] }} 日</span></div>
        <n-spin :show="loadingValidation" size="small">
          <div v-if="validationPreview?.available && validationPanels.length" class="validation-panels">
            <article v-for="panel in validationPanels" :key="panel.key" class="validation-panel">
              <header><div><strong>{{ panel.label }}</strong><small>样本 {{ panel.data.sample_count }} · {{ relationshipLabel(panel.data.relationship) }}</small></div><n-tag :type="relationshipType(panel.data.relationship)" size="small" :bordered="false">{{ relationshipLabel(panel.data.relationship) }}</n-tag></header>
              <div class="validation-result"><span>分数与末日广度秩相关 <b>{{ outcomeText(panel.data.market_breadth_rank_correlation, '', 3) }}</b></span><span>分数与核心指数收益秩相关 <b>{{ outcomeText(panel.data.core_index_return_rank_correlation, '', 3) }}</b></span></div>
              <div class="bucket-grid"><div v-for="bucket in panel.data.buckets" :key="bucket.code" class="bucket"><strong>{{ bucket.label }} {{ bucket.score_range }}</strong><small>{{ bucket.sample_count }} 个样本</small><span>末日广度 <b>{{ outcomeText(bucket.average_market_breadth_pct, '%') }}</b></span><span>核心指数累计 <b>{{ outcomeText(bucket.average_core_index_cumulative_return_pct, '%', 2) }}</b></span></div></div>
              <p class="bucket-note">高分 − 低分：末日广度 {{ signedOutcome(panel.data.high_low_difference.breadth_pct_point_difference, ' 个百分点') }}；核心指数累计 {{ signedOutcome(panel.data.high_low_difference.core_index_return_pct_point_difference, '%', 2) }}</p>
            </article>
          </div>
          <n-alert v-else type="info" :show-icon="true" class="validation-empty">{{ validationPreview?.reason === 'market_emotion_not_calculated' ? '该模型尚未有可验证的日度评分；完成基线校准后会自动出现。' : '正在读取已持久化评分的历史环境验证。' }}</n-alert>
        </n-spin>
        <p class="panel-note">“T+3 核心指数累计”是从 T 后首个开市日到第 3 个开市日的七个核心指数日均涨跌幅复利累计值；“末日广度”是对应目标日的上涨比例。它检验情绪分对市场环境的历史区分度，不等于候选股票收益、胜率或买卖建议。若高分与低分关系为反向或混合，应先在草稿模型中研究参数，不自动调权。</p>
      </section>
...
import type { MarketEmotionDaily, MarketEmotionModel, MarketEmotionValidationHorizon, MarketEmotionValidationPreview } from '@/types/market-insight';
...
const validationPreview = ref<MarketEmotionValidationPreview | null>(null);
...
const validation = computed(() => validationPreview.value?.validation);
const validationPanels = computed<Array<{ key: string; label: string; data: MarketEmotionValidationHorizon }>>(() => {
  const value = validation.value;
  if (!value) return [];
  return [
    { key: 'short-t1', label: '短线接力分 · T+1', data: value.short_term.t_plus_1 },
    { key: 'short-t3', label: '短线接力分 · T+3', data: value.short_term.t_plus_3 },
    { key: 'risk-t1', label: '大盘风险偏好分 · T+1', data: value.risk_on.t_plus_1 },
    { key: 'risk-t3', label: '大盘风险偏好分 · T+3', data: value.risk_on.t_plus_3 },
  ];
});
...
function outcomeText(value: number | null | undefined, suffix = '', digits = 2) { return typeof value === 'number' ? `${value.toFixed(digits)}${suffix}` : '-'; }
function signedOutcome(value: number | null | undefined, suffix = '', digits = 2) { return typeof value === 'number' ? `${value >= 0 ? '+' : ''}${value.toFixed(digits)}${suffix}` : '-'; }
function relationshipLabel(value: string) { return ({ positive: '正向区分', inverse: '反向关系', mixed: '关系混合', insufficient_samples: '样本不足', insufficient_outcomes: '结果不足' } as Record<string, string>)[value] || '待验证'; }
function relationshipType(value: string) { return ({ positive: 'success', inverse: 'error', mixed: 'warning', insufficient_samples: 'default', insufficient_outcomes: 'default' } as Record<string, 'success' | 'error' | 'warning' | 'default'>)[value] || 'default'; }
...
async function loadValidation(model: MarketEmotionModel) {
  const requestSequence = ++validationRequestSequence;
  loadingValidation.value = true;
  validationPreview.value = null;
  try {
    const response = await marketInsightApi.emotionModelValidation(model.model_code, { history_limit: Math.max(60, Math.min(model.baseline_trade_days, 1000)) });
    if (requestSequence === validationRequestSequence) validationPreview.value = response;
  } catch (error) {
    if (requestSequence === validationRequestSequence) message.error(error instanceof Error ? error.message : '加载历史环境验证失败');
  } finally {
    if (requestSequence === validationRequestSequence) loadingValidation.value = false;
  }
}
