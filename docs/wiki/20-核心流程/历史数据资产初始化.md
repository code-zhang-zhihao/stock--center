...
先使用目标表 owner 或数据库管理员账号执行 `docs/sql/55-data-asset-history-pipelines.sql`，再依次执行仅包含 DML 的 `docs/sql/56-sector-history-range-backfill.sql` 和 `docs/sql/57-stock-history-limit-events.sql`，最后 reload Scheduler。仅有 DML 读写权限的应用账号无法对已有表执行 `ALTER TABLE`，但可以执行 `56/57` 更新既有任务定义。`55` 会 seed 上述 6 个任务、增加 `t_stock_factor_daily.ma30/ma60`、创建 `t_index_factor_daily`，并删除 7 个已被替换的旧任务定义；`t_scheduler_job_run` 中的旧运行记录不删除。
...
`backfill_stock_daily_facts` 依次运行 5 个事实阶段。前 4 个阶段按股票队列处理，每只股票对对应 Tushare 接口只发起一次完整 `start_date/end_date` 区间请求；第 5 个事件阶段改为按交易日查询全市场，避免逐股票放大请求：
...
任务默认 `append_safe + only_missing=true`，可重复续跑。前 4 个事实阶段按业务键日期判断单股缺口；事件阶段在 `t_provider_raw_record` 为成功交易日写入范围完成标记，即使当天两个接口都返回 0 行也能在下次跳过。`max_stocks` 会同时限制逐股事实和事件入库范围，并使用独立完成范围，不会把 smoke 标记误认为全股票池已完成。单股或单事件交易日失败进入结果摘要，后续队列继续；下一次运行会重试缺口。`rebuild` 会先删除指定股票池和日期范围内的 5 类事实再重新拉取，应只在明确需要全量重建时使用。
