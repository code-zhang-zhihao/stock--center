...
执行顺序固定为：`77 prepare → 78 backfill → 停 worker → 79 cutover → 部署正式代码 → 79a 板块窗口任务升级 → 三类有界历史因子回填 → 79b resume → 观察 7 日并备份 → 80 cleanup`。
...
- `79a` 只更新历史板块因子任务的参数和说明，不改任何因子行；历史板块因子随后按实际行情日期、20 个交易日窗口执行。
...
- `docs/sql/79a-sector-factor-window-backfill.sql`
