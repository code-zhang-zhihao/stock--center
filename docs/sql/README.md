...
> 日频数据资产的当前生产契约以 `77–80` 和下文“日频数据资产最终收口执行手册”为准。`01–76` 仍保留为历史演进与已建库升级证据；一旦执行 `79` 完成正式切换，不得再单独执行会恢复旧因子、旧情绪、专业 JSON 或 Raw 依赖的早期脚本。
...
77. `77-daily-assets-final-prepare.sql`：创建无版本后缀正式因子影子表、板块龙头、市场汇总及单位明确的新列，并安装事实双写触发器；不停任务、不改正式表名、不删除数据。
78. `78-daily-assets-final-backfill.sql`：按 20 个交易日窗口迁移正式因子和窄事实单位，抽取 `stk_factor_pro` 的 78 个 QFQ 类型列及 4 个计数列；历史 JSON 中以 `0.0/13.0` 数值形式保存的计数会先转 `numeric` 再转 `integer`。脚本同时修正同花顺板块资金“亿元→元”，并按交易日重建最近 30 日分钟因子；可中断续跑，某窗口失败时该窗口整体回滚，替换修正版脚本后可直接重跑。
79. `79-daily-assets-final-cutover.sql`：维护窗口内停用每日任务、完成最近 20 个交易日追平和抽样校验，原子切换 `t_stock_factor_daily`，删除运行时 V1/V2 视图依赖；脚本结束后任务保持停用。
80. `79b-daily-assets-final-resume.sql`：部署正式代码并完成个股/板块/指数历史派生回填后验证最新日覆盖、七大 canonical 指数代码（库内为无后缀六位代码）、市场汇总和审计，再恢复切换前处于启用状态的每日任务并进入 7 日观察期。
81. `80-daily-assets-final-cleanup.sql`：观察至少 7 日且已完成数据库备份后，删除旧因子、专业 JSON、技术快照、旧情绪、Raw 表和冗余列，并清理因子字典中的资产版本残留。该脚本不可逆。
...
`77–80` 是一组完整迁移，不允许只执行改名脚本：

1. 用数据库表空间所在磁盘核对可用空间并完成备份准备，然后在线执行 `77`。
2. 在线执行可恢复的 `78`；每 20 个交易日独立提交，失败后直接重跑，不会回滚已完成窗口。
3. 在维护窗口停止/暂停 Scheduler worker，确认相关运行均已结束，再执行 `79`。`79` 会保持每日任务禁用。
4. 部署并重启本次正式资产代码；手动运行个股、板块、指数三个有界历史因子任务。个股任务会同步重建市场汇总。
5. 验证历史任务结束后执行 `79b`。覆盖门禁通过才会恢复切换前原本启用的每日任务，并开始 7 日生产观察。
6. 观察期核对每日覆盖、策略候选差异、19:30 耗时和 API P95；确认数据库备份可恢复后才执行不可逆的 `80`。

不得在 `79` 与新应用部署之间重新启动旧 worker；否则旧 ORM 会面对已经改名的表。`80` 前保留 `t_stock_factor_daily_legacy_final`、专业 JSON 和 Raw 表作为回滚证据，不得手工提前删除。
