...
def normalize_search_result(result: Any):
    """兼容新版 dict 返回和旧版三元组返回。"""
    if isinstance(result, tuple) and len(result) == 3:
        return result
    if not isinstance(result, dict):
        return False, [], "搜索返回格式异常"
    ...
    if isinstance(raw_response, dict):
        ...
        if isinstance(data, list):
            ...
            for key in ("datas", "results", "result", "items", "list"):
                value = data.get(key)
    ...
    if result.get("success"):
        return True, rows[: args.limit], "搜索成功" if rows else "未找到相关公告"
    return False, [], result.get("error") or f"搜索失败，状态码: {result.get('status_code')}"

def output_json(query: str, success: bool, results: List[Dict[str, Any]], message: str):
    print(
        json.dumps(
            {
                "success": success,
...
def main():
    ...
    if args.input:
        ...
        for query, raw_result in batch_results.items():
            success, results, message = normalize_search_result(raw_result)
            if success:
        ...
        success, results, message = normalize_search_result(search.search(args.query, args.limit))

        execution_time = Utils.calculate_execution_time(start_time)

        if args.format == "json" and not args.output:
            output_json(args.query, success, results, message)
            if not success:
                sys.exit(1)
            sys.exit(0)
