...
它们只使用已入库的日 K、正式日频因子、`daily_basic`、个股资金流、涨跌停/炸板事件、概念关系和正式双分情绪结果。共同排除 ST、北交所、非 active 股票、上市不足 60 个交易日及流动性不足标的；默认阻断冰点/退潮市场。模板给出的默认止损、分批止盈、移动止盈和最长持有期只是可验证的模拟规则，不是买卖建议，也不代表胜率。

尚未接入统一日频计算器的 MACD、RSI、KDJ、BOLL 等策略不会以名称形式伪装成可运行模板；先补齐指标、回测与测试，才能注册新实现。

策略、历史回测和情绪趋势扩散统一读取正式 `t_stock_factor_daily`。OHLC、前收和技术指标均为 QFQ，突破、均线排列、回踩以及历史高低点不得混入 BFQ 价格。个股日 K 页面仍展示 `t_daily_bar` 的 BFQ K 线，策略价格判断只使用正式 QFQ 列。
...
`run_strategy_backtest` 是日频、单信号独立样本基线：T 收盘筛选，T+1 开盘使用正式因子表的 QFQ 开盘价与滑点模拟入场，随后以同口径每日收盘判定止损/止盈/移动止盈/期限，下一开市日开盘模拟退出。BFQ 与 QFQ 不能在同一笔交易中混用。它不模拟组合资金容量、盘口排队、撮合、停牌可成交性或真实订单，因此只能用于比较策略版本，不能声称为实盘收益预测。

回测按 5 个信号日分批提交，幂等键是版本、区间、费率、滑点和研究候选上限；运行摘要先显示 `loading_inputs`，再显示已完成信号日数。固定模板只加载当前 bar 加必要历史窗口；数据库先以规则必要条件预筛选，再返回候选日正式因子和按股票聚合的最小价格序列。事件模板的涨停题材上下文读取同日 `t_sector_factor_daily` 并缓存概念成员关系，不为每批重复做全市场大关联，也不带入未来题材关系。最终命中仍由 Python evaluator 判定。汇总只投影收益字段；缺失交易日按实际日期过滤。寻优基线必须使用足够大的候选上限，只有满足样本日、结束交易数、胜率和正平均收益门槛的正式回测才可进入人工纸面审阅。取消运行会删除已提交的部分交易明细并保留 `cancelled` 运行审计，不能作为寻优或纸面审阅输入。

### 历史参数寻优

`optimize_strategy_parameters` 是人工触发的研究任务，不是“找一个历史最高胜率数字”的功能。它必须引用同一不可变版本的一条已完成基线回测，并先检查该回测没有触及 `selection.max_candidates`：若某日候选已被上限截断，收紧参数后可能本应保留原先未落入基线的股票，因此任务会拒绝运行，要求把候选上限调高后重跑完整基线。参数复放只读取每笔交易的信号日、净收益和冻结候选快照，不读取入/出场及执行快照；这不改变 trial 的事实输入或排名，只减少高频基线在多轮调优时的无关 JSON 传输。
