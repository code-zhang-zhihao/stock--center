...
新库按顺序执行 `55/56/57` 后，再由目标表 owner 执行 `docs/sql/72-stock-daily-asset-convergence-v2.sql`、`75-stock-factor-v2-window-performance.sql` 和 `76-stock-factor-v2-price-basis-consumers.sql`，随后 reload Scheduler。`72/75/76` 只新增或优化影子资产、视图和任务定义，不删除旧事实；V1 保持 active、V2 保持 shadow。完成 2020-01-01 起的个股事实预热、2021-01-01 起的 V2 因子回填以及至少 5 个完整交易日影子观察后，只有门禁通过才执行 `73-activate-stock-daily-factor-v2.sql`。需要语义回滚时执行 `74-rollback-stock-daily-factor-v1.sql`。
...
| `v_stock_factor_daily_active_basis` | Read view | 激活版本业务键 | V1/V2 因子表 | 同口径价格消费；V1 由消费者回退 BFQ 日 K，V2 同时提供 QFQ OHLC 与 QFQ 因子。 |
...
2. 每个窗口以 `sql_stock_chunk_size=200` 划分股票范围，使用 `calculation_workers=2` 个独立数据库会话并发计算；可按数据库余量在 1–4 间调整，每个分片独立提交。
3. PostgreSQL 从窄事实、复权历史和专业档案组装类型化列，幂等写 `t_stock_factor_daily_v2`。`fund_strength_percentile` 不在每个股票分片中重复扫描全市场，而是在该日期窗口的股票分片全部完成后统一刷新一次。
4. `append_safe + only_missing=true` 只跳过 `factor_status=ready` 的 V2 行；`partial` 会在事实补齐后重新组装。窗口或进程中断时保留已提交分片，使用相同参数重跑即可续算。
...
回填完成后先查看 `v_stock_factor_daily_v2_validation`，并连续观察 5 个完整交易日。覆盖率、除权日前后价格连续性、页面动态快照和策略回测都通过后才执行 `73`。`73` 的实际门禁按最近 5 日逐日走索引检查 `ready` 覆盖和复权回退，不再全量扫描 V1/V2 比较视图；切换后价格比较消费者通过 `v_stock_factor_daily_active_basis` 读取 V2。发现语义问题可执行 `74` 恢复 V1。
