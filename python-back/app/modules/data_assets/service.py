...
ASSET_DEFINITIONS: tuple[AssetDefinition, ...] = (
    ...
    asset("daily_bars", "个股日线", "daily_fact", "t_daily_bar", "daily", "核心收盘", ["daily_close_core_ingest", "backfill_stock_daily_bars"], "trade_date", None, "stock_code", expected_lag_trade_days=2, approximate_row_count=True, coverage_scope="active_stock_daily"),
    asset("daily_basic", "每日估值与流动性", "daily_fact", "t_stock_daily_basic", "daily", "核心收盘", ["daily_close_core_ingest", "backfill_stock_daily_basic"], "trade_date", None, "stock_code", expected_lag_trade_days=2, coverage_scope="active_stock_daily"),
    asset("stock_fund_flow", "个股资金流", "daily_fact", "t_stock_fund_flow_daily", "daily", "核心收盘", ["daily_close_core_ingest", "backfill_stock_moneyflow"], "trade_date", None, "stock_code", expected_lag_trade_days=2, coverage_scope="active_stock_daily"),
    asset("limit_events", "涨跌停与停复牌", "daily_fact", "t_limit_event_daily", "daily", "核心收盘", ["daily_close_core_ingest"], "trade_date", None, "stock_code", expected_lag_trade_days=2),
    ...
    asset("index_bars", "核心指数日线", "daily_fact", "t_index_bar", "daily", "核心收盘", ["daily_close_core_ingest"], "trade_date", None, "index_code", expected_lag_trade_days=2),
    asset("index_daily_basic", "指数每日指标", "daily_fact", "t_index_daily_basic", "daily", "晚间增强", ["daily_close_enrichment_ingest", "daily_close_repair_ingest"], "trade_date", None, "index_code", expected_lag_trade_days=5),
    ...
    asset("sector_bars", "板块日线", "daily_fact", "t_sector_bar", "daily", "核心收盘", ["daily_close_core_ingest"], "trade_date", None, "sector_code", expected_lag_trade_days=2),
    asset("sector_fund_flow", "板块资金流", "daily_fact", "t_sector_fund_flow_daily", "daily", "晚间增强", ["daily_close_enrichment_ingest", "daily_close_repair_ingest"], "trade_date", None, "sector_code", expected_lag_trade_days=5),
    asset("stock_technical_factor", "Tushare 专业技术因子", "daily_fact", "t_stock_technical_factor_daily", "daily", "晚间增强", ["daily_close_enrichment_ingest", "daily_close_repair_ingest"], "trade_date", None, "stock_code", expected_lag_trade_days=5, coverage_scope="active_stock_daily"),
    ...
    asset("stock_factor_daily", "个股日频因子", "derived", "t_stock_factor_daily", "daily", "核心/增强因子", ["daily_close_core_ingest", "daily_close_enrichment_ingest", "daily_close_repair_ingest", "backfill_daily_factors"], "trade_date", None, "stock_code", expected_lag_trade_days=2, coverage_scope="active_stock_daily"),
    ...
    asset("technical_snapshots", "技术快照", "derived", "t_technical_indicator_snapshot", "eod", "核心因子/修复", ["daily_close_core_ingest", "backfill_technical_snapshots"], None, "snapshot_time", "stock_code", expected_lag_trade_days=3, coverage_scope="active_stock_daily"),
    asset("sector_factor_daily", "板块日频因子", "derived", "t_sector_factor_daily", "daily", "核心/增强因子", ["daily_close_core_ingest", "daily_close_enrichment_ingest", "daily_close_repair_ingest", "backfill_sector_factors"], "trade_date", None, "sector_code", expected_lag_trade_days=3),
)
...
class DataAssetsService:
    ...
    async def cached_summary(self) -> DataAssetSummary:
        payload = await self._read_cache(SUMMARY_SNAPSHOT_KEY)
    ...
    async def cached_daily_health(self, days: int = 3) -> DataAssetDailyHealthReport:
        ...
        payload = await self._read_cache(cache_key)
        if isinstance(payload, dict):
            return DataAssetDailyHealthReport.model_validate(payload)
        report = await self.daily_health(days=days)
        await self._write_success_snapshot(cache_key, report.model_dump(mode="json"))
        return report
    ...
    async def refresh_cache(self, days: int = 3, snapshot_key: str = "all") -> DataAssetRefreshResult:
        ...
        if DAILY_HEALTH_SNAPSHOT_KEY in requested:
            try:
                ...
                failed.append(DAILY_HEALTH_SNAPSHOT_KEY)
                await self._write_failed_snapshot(DAILY_HEALTH_SNAPSHOT_KEY, exc)
        ...
        for definition in ASSET_DEFINITIONS:
            ...
            if not stats.exists:
                ...
                if definition.expected_lag_trade_days is not None:
                    ...
                    status = self._status("ok")
            if stats.exists and stats.row_count > 0 and definition.coverage_scope:
                ...
                coverage = await self.repository.stock_daily_coverage(definition, coverage_date)
    ...
    async def _write_success_snapshot(self, snapshot_key: str, payload: dict) -> None:
        ...
        if cache_config.data_asset_cache_enabled:
            await redis_client.set_json(
                await self._payload_key(snapshot_key),
                payload,
    ...
    async def _cache_status_item(self, snapshot_key: str) -> dict:
        ...
        if not isinstance(meta, dict):
            return {
                ...
                "refreshed_at": None,
                "is_stale": True,
                "error_message": "缓存为空或不可用，请手动刷新或等待调度预热。",
            }
        return {
            "snapshot_key": snapshot_key,
