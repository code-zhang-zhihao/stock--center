...
`stock-center` 必须把“实时数据、准实时数据、盘后数据、历史数据”明确分层。TickFlow 的核心体验里有实时 quote 缓存、SSE 推送、盘口深度和盘中监控；实时数据底座 V1 已落地统一的 `RealtimeMarketService`，后续页面和监控不得各自直接打 MooTDX、AkShare 或 Skill。
...
| 能力 | 当前实现 | 实时性 | 问题 |
| --- | --- | --- | --- |
| 个股实时 quote | `RealtimeMarketService` 的全市场 MooTDX `quote_batch` 缓存 | 60 秒准实时 | Redis/内存覆盖缓存，SSE 推送给当前详情页；不写 PostgreSQL。 |
...
- FastAPI 生命周期启动 `RealtimeMarketService`；配置对象为 `market_data/realtime_market`，默认 `enabled=false`，防止未配置 Redis/MooTDX 时服务启动即发起盘中轮询。
- 只在 `t_trade_calendar` 标记为 CN 开市且位于 `09:30-11:30`、`13:00-15:00` 的时段自动运行；手动 `POST /api/v1/realtime/refresh?force=true` 可用于诊断。
- 全市场 quote 采用 MooTDX `quote_batch`，严格 80 只一批、默认 60 秒一轮。缓存覆盖写入 `realtime:quotes`、`realtime:market-overview`、`realtime:sectors`、`realtime:pools`，TTL 默认 180 秒。
- 分钟线采用 4 个相互独立的 `MootdxProvider` 连接池调用 `minute(symbol)`。目标登记最多 500 只；持仓池、重点监控池、当前打开的个股页面优先，其余由强势候选轮转。Quote 临时降级时使用 active 股票范围补足登记目标，分钟线仍独立执行；每轮最多选 200 只，实测 3 轮均完成 200/200。
...
| --- | --- | --- | --- |
| MooTDX | 个股 quote、指数 quote、分时、盘口、分笔 | 财务、板块资金、公告 | 实时行情主源。 |
...
```mermaid
flowchart LR
  A["MooTDX 实时行情"] --> D["RealtimeMarketService"]
  B["AkShare/东财准实时排行"] --> D
  C["本地股票池/板块范围"] --> D
  D --> E["内存 Quote Cache"]
  D --> F["内存 Minute Cache"]
...
- 全市场 quote：沪深 active 股票，当前约 5210 只；使用 MooTDX `quote_batch` 分批轮询。
...
- `quote_batch` 可以作为第一版全市场 quote 主源。
- `batch_size` 必须默认设为 80，不能设 100/200/500。
- 压测发生在非交易时段，盘中还需要 30-60 分钟 soak test 验证断连、空返回和缺失率。
- 全市场默认不要低于 60 秒一轮；30 秒只能作为激进配置。

建议默认配置：

| 参数 | 默认值 | 说明 |
| --- | --- | --- |
| `full_market_batch_size` | 80 | MooTDX 单批安全上限。 |
| `full_market_interval_seconds` | 60 | 全市场 quote 默认刷新间隔。 |
| `aggressive_interval_seconds` | 30 | 激进模式，可手动开启。 |
| `watchlist_interval_seconds` | 15-30 | 人工股票池数量少，可以更快。 |
| `request_gap_seconds` | 0.05-0.10 | 批次间轻微间隔，降低通达信服务器压力。 |
| `stale_after_seconds` | 180 | 超过该时间未成功刷新则标记 stale。 |
| `round_failure_threshold` | 5% | 本轮缺失超过阈值时保留上一轮缓存并降级提示。 |

全天请求量估算：

| 全市场刷新间隔 | 交易时段约 240 分钟 | 请求量 |
| --- | ---: | ---: |
| 60 秒 | 240 轮 × 66 批 | 约 1.58 万次 |
| 30 秒 | 480 轮 × 66 批 | 约 3.17 万次 |
| 15 秒 | 960 轮 × 66 批 | 约 6.34 万次，不建议全市场使用 |
...
t_stock_pool / t_stock_pool_member
    +
RealtimeMarketService.quote_cache
    +
