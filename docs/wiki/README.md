...
| 策略研究与模拟交易 | `python-back/app/modules/strategy_center/` | `service.py`、`repository.py`、`api.py` | 策略草稿、专属动态策略池、候选状态与模拟交易审计模型；当前不含 evaluator、实时确认或自动交易。 | [策略中心模块](30-模块地图/策略中心模块.md) |
